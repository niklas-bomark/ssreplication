...
for (period in time_periods){
    ...
    directory_path <- "output/tables/"
    name <- "table_residwage_replication_"
    modified_period <- gsub(":", "_", period)
    filename <- paste0(directory_path, name, modified_period, ".tex")
    ...
    if (period == 1998)  {
        ...
        res_out %>%
            ...
            geom_pointrange(size = 1, lwd = 1.65, position =  position_nudge(y = -0.1))+
            geom_pointrange(aes(x=ss_estimate, y= term, xmin = ss_conf.low, xmax = ss_conf.high),color = "red", size = 1, lwd = 1.65, position =  position_nudge(y = 0.1))+#position = position_jitter(width = 0.25))+
        ...
        name_figure <- "resvar_replication_"
        modified_period <- gsub(":", "_", period)
...
for (period in time_periods){
    ...
    directory_path <- "output/tables/"
    name <- "table_residwage_extension_"
    modified_period <- gsub(":", "_", period)
    filename <- paste0(directory_path, name, modified_period, ".tex")
    write(tex8, file=filename)
        ...
        res_out %>%
         ...
         # geom_pointrange(aes(x=ss_estimate, y= term, xmin = ss_conf.low, xmax = ss_conf.high),color = "red", size = 1, lwd = 1.65,position = position_jitter())+#position = position_jitter(width = 0.25))+
          ...
          theme(#panel.background.x= element_rect(fill = NA, color = "black"),
            ...
            plot.title = element_text(size = 130,
                                      color = "black"),
            axis.text.x=element_text(size=12.50, 
                                     color = "black"),
            axis.text.y=element_text(size=15, 
                                     color = "black"),
            axis.title.x =element_text(size=25,
...
for (period in time_periods){
    ...
    #Name and save
    directory_path <- "output/tables/"
    name <- "table_grosswage_replication_"
    modified_period <- gsub(":", "_", period)
    filename <- paste0(directory_path, name, modified_period, ".tex")
    write(tex8, file=filename)
...
for (period in time_periods){
  ...
  #Name and save
  directory_path <- "output/tables/"
  name <- "table_grosswage_extension_"
  modified_period <- gsub(":", "_", period)
  filename <- paste0(directory_path, name, modified_period, ".tex")
  write(tex8, file=filename)
